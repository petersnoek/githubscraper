...
class FileCache {
    ...
    public function hasItem($key, $max_age_seconds = 0) {
        //echo 'Searching cache for ' . $key . ' with max age: ' . $max_age_seconds . '<br>';
        $filename = $this->cacheLocation . $this->Sanitize($key);
        if ( file_exists($filename) ) {
            if ( $max_age_seconds == 0 ) {
                //echo 'Found ' . $key . '<br>';
                return true;    // file found and no aging limit
            } else {
                if ( time()-filemtime($filename) > $max_age_seconds ) {
                    // file found but too old
                    //echo 'Too old: ' . $key . '<br>';
                    return false;
                } else {
                    //echo 'Still valid: ' . $key . '<br>';
                    return true;
                }
            }
        } else {
